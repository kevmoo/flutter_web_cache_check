...
| `F-05`  | SPA Catch-All Rewrite Poisoning                                    | `fail` / `ok`          | Probes `/main.dart.00000000.js`, `/main.dart.00000000.wasm`, and `/assets/__cache_check_missing__.00000000.png` (plus guards `/flutter_bootstrap.js` and manifest probes). Fails if any returns `200` / HTML. |
...
| `W-01`  | WebAssembly (`dart2wasm`) Target Detection                         | `warn` / `ok`          | Verifies `_flutter.buildConfig` in `flutter_bootstrap.js` includes a `dart2wasm` (`mainWasmPath`) target alongside `dart2js` (warns when missing unless `--no-wasm` is passed).                               |
