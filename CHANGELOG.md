...
- Update `fb-config` to emit the 5-rule `last-match-wins` `firebase.json` header stack (`max-age=0, must-revalidate` base + `404.html` guard + `assets/**` immutable with unhashed manifest override) and tighten unguarded `"**"` SPA rewrites to `!(/assets/**|/canvaskit/**|/icons/**|/main.dart.*)`.
