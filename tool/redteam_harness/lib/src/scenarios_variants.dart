...
/// way a user would need it to across both JS and Wasm builds?
class CacheCheckDogfoodScenario extends Scenario {
  @override
  String get id => 'S15';
  @override
  String get title =>
      'flutter_web_cache_check verdicts per hosting policy (JS + WASM)';
  @override
  String get description =>
      'Runs UrlChecker in-process against hashed JS and Wasm v1 deploys under every HeaderPolicy.';

  static const Set<HeaderPolicy> _shouldPass = <HeaderPolicy>{
    HeaderPolicy.strict,
  ...
  Future<void> run(ScenarioContext ctx, ScenarioResult result) async {
    ...
    await _verifyJsDeploy(ctx, result, jsBuild);
    await _verifyWasmDeploy(ctx, result, wasmBuild);
  }

  Future<void> _verifyJsDeploy(
    ScenarioContext ctx,
    ...
    try {
      ...
      final CheckReport warnReport = await UrlChecker(host.baseUri.toString())
          .analyze();
      final Step warnStep = result.step('JS deploy (requireWasm: true)');
      warnStep.check(
        'emits W-01 warning when Wasm is missing',
        warnReport.findings.any(
          (CheckFinding f) => f.ruleId == 'W-01' && f.severity == Severity.warn,
        ),
        warnReport.findings.map((CheckFinding f) => f.toString()).join(' | '),
      );
    } finally {
      await host.stop();
    }

    for (final HeaderPolicy policy in HeaderPolicy.values) {
      await _verifyPolicyWithBuild(
        ctx: ctx,
        result: result,
        build: jsBuild,
        policy: policy,
  ...
  Future<void> _verifyWasmDeploy(
    ScenarioContext ctx,
    ScenarioResult result,
    BuildResult wasmBuild,
  ) async {
    for (final HeaderPolicy policy in HeaderPolicy.values) {
      await _verifyPolicyWithBuild(
        ctx: ctx,
  ...
  Future<void> _verifyPolicyWithBuild({
    ...
    try {
      ...
      final Step step = result.step('$labelPrefix policy ${policy.name}');
      final bool expectedPass = _shouldPass.contains(policy);
      final bool passed = !report.hasErrors && !report.hasWarnings;
      step.check(
        expectedPass
  ...
  void _checkExpectedRulesOk(Step step, CheckReport report) {
    for (final String ruleId in <String>[
      ...
      step.check(
        '$ruleId is Severity.ok',
        hasOk,
        hasOk ? '' : 'missing $ruleId ok',
      );
    }
  }
}
