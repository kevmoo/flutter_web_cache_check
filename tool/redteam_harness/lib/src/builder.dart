...
class FlutterSdk {
  ...
  static Directory _normalizeSdkRoot(String rawPath) {
    final String cleaned = p.normalize(rawPath);
    if (cleaned.endsWith(p.join('bin', 'flutter'))) {
      return Directory(cleaned).parent.parent;
    }
...
class Builder {
  ...
  Future<BuildResult> build(
    ...
    _prepareSourceVariant(srcDir, options);

    final args = _buildFlutterArgs(options, outDir);
  ...
  static void _prepareSourceVariant(Directory srcDir, BuildOptions options) {
  ...
  static List<String> _buildFlutterArgs(
    BuildOptions options,
    Directory outDir,
  ) => <String>[
    'build',
