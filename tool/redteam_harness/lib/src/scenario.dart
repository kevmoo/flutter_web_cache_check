...
class ReportWriter {
  ...
  static String markdown(
    ...
    for (final r in results) {
      b.writeln(_summaryRow(r));
    }
    for (final r in results) {
      _writeScenarioSection(b, r);
    }
    return b.toString();
  }

  static String _summaryRow(ScenarioResult r) {
    ...
    final detail = r.error != null
        ? 'BLOCKED: ${r.error!.split('\n').first}'
        : failing;
    return '| ${r.id} ${r.title} | ${r.sdk} | ${r.verdict.name.toUpperCase()} | $detail |';
  }

  static void _writeScenarioSection(StringBuffer b, ScenarioResult r) {
    b.writeln(
      '\n## ${r.id} ${r.title} (`${r.sdk}`) — ${r.verdict.name.toUpperCase()}\n',
    );
    for (final s in r.steps) {
      _writeStepSection(b, s);
    }
    if (r.error != null) b.writeln('```\n${r.error}\n```');
  }

  static void _writeStepSection(StringBuffer b, Step s) {
    b.writeln('### ${s.passed ? '✅' : '❌'} ${s.title}\n');
    for (final c in s.checks) {
      final suffix = c.detail.isEmpty ? '' : ' — ${c.detail}';
      b.writeln('- ${c.passed ? '✅' : '❌'} ${c.name}$suffix');
    }
